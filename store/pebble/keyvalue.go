...
// errors.As(err, &pebble.ErrStoreWrite{}) or errors.Is(err, ErrStoreWriteSentinel).
//
...
// errors.Is(err, pebble.ErrStoreWriteSentinel) returns true for any ErrStoreWrite
...
//
// The composite-key format (WindowCompositeKey) is owned by store/pebble; this
// method decodes it so callers outside the package do not need to import store/pebble
// just to enumerate sessions. (gstream cannot import store/pebble: store/pebble
